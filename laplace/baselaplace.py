...
class ParametricLaplace(BaseLaplace):
        ...
        if pred_type == 'glm':
            ...
            if link_approx == 'mc':
                return self.predictive_samples(x, pred_type='glm', n_samples=n_samples, 
                                               diagonal_output=diagonal_output).mean(dim=0), f_mu,f_var         #MODIFSAMANDINE   # notes amandine - authors use mean here ! 
    ...
    def predictive_samples(self, x, pred_type='glm', n_samples=100, 
        ...
        if pred_type == 'glm':
            ...
            f_samples = normal_samples(f_mu, f_var, n_samples, generator)           # notes amandine - there you have your predictions distributions 
            if self.likelihood == 'regression':
                return f_samples
            return torch.softmax(f_samples, dim=-1)                                 # then return softmax of distribution
